...
gulp.task('dist-clean-js', function () {
	return gulp.src(conf.path.dist.js + '/*', { read: false }).pipe(clean({ force: true }));
});

gulp.task('build-css', ['dist-clean-css'], function () {
	return gulp
		.src(conf.path.build.css + '/*.css')
		.pipe(ver())
...
gulp.task('clean-css', function () {
	return gulp.src(conf.path.build.css + '/*', { read: false }).pipe(clean({ force: true }));
});
...
gulp.task('clean-js', function () {
	return gulp.src(conf.path.build.js + '/*', { read: false }).pipe(clean({ force: true }));
});
...
gulp.task('clean-html-template', function () {
	return gulp.src(conf.path.build.html + '/*.html', { read: false }).pipe(clean({ force: true }));
});
